...
namespace BSP.ViewModels
{
    public class MainWindowViewModel : BaseNotificationViewModel
    {
        ...
        #region Properties
        public int Precision { get => precision; set { precision = value; OnChanged(); } }
        ...
        public bool ValidateInputs()
        {
            ...
            if (!hasSelectedRadionuclides && SourceTab.IsAutoGeneratedModeChecked)
                base.AddError((res["msg_Error_NoSelectedRadionuclides"] as string) ?? "No one radionuclide was selected for calculations");

            if (SourceTab.SourceDimensions.Select(d => d.Discreteness).Count(d => d == 0) > 0)
                base.AddError((res["msg_Error_DimensionDiscreteness"] as string) ?? "Dimension discreteness should be greated zero");

            if (SourceTab.SourceDimensions.Select(d => d.Value).Count(d => d == 0) > 0)
                base.AddError((res["msg_Error_ZeroDimension"] as string) ?? "Dimension should be greated zero");

            if (SourceTab.EnergyYieldList.Count == 0)
                base.AddError((res["msg_Error_EmptyEnergyYieldData"] as string) ?? "Energy and Yield data are empty. Click update to generate data or enter it manually");

            if (SourceTab.SourceTotalActivity < 1)
                base.AddError((res["msg_Error_ZeroTotalActivity"] as string) ?? "Source total activity is zero or empty");

            if (CalculationDistance <= 0)
                base.AddError((res["msg_Error_ZeroCalculationDistance"] as string) ?? "Calculation distance have to be greater zero");

            if (Precision <= 0)
                base.AddError((res["msg_Error_ZeroOrLessDecimalPlaces"] as string) ?? "Number of decimal places is less or equal zero");
        ...
        public async Task StartCalculation()
        {
            ...
            else
            {
                ...
                MessageBox.Show(
                    ...
                    "\n" + base.ValidationMessagesToString());
            }
        }
        #endregion
        ...
        private void FillOutputTable(OutputValue results)
        {
            ...
            string partialDataFormat = "\t{0,12:G3}{1,15:e3} {2}\n";
