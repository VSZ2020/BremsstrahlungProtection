...
namespace BSP.ViewModels.Tabs
{
    public class SourceTabVM : BaseViewModel, IDataErrorInfo
    {
        ...
        private void ImportEnergyIntensityData()
        {
            ...
            {
                var items = ReadEnergyIntensityFile(openDialog.FileName, openDialog.FileName.EndsWith(".txt") ? '\t' : _CSV_DELIMETER);
                if (items.Count() > 0)
                {
        ...
        private void ExportEnergyIntensityData()
        {
            ...
            if (result.HasValue && result.Value)
            {
                ...
                if (filename.EndsWith(".txt"))
                    ExportEnergyIntensityToTextFile(filename);
                else
                    ExportEnergyIntensityToTextFile(filename, _CSV_DELIMETER);
            }
        }
        #endregion

        #region ExportEnergyIntensityToTextFile
        private void ExportEnergyIntensityToTextFile(string filename, char delimeter = '\t')
        {
            using (StreamWriter wr = new StreamWriter(filename))
            {
                ...
                {
                    wr.WriteLine(string.Join(delimeter, EnergyYieldList[i].Energy, EnergyYieldList[i].EnergyYield, EnergyYieldList[i].EnergyFlux, EnergyYieldList[i].PhotonsFlux, SourceTotalActivity));
                }
            }
        ...
        private IEnumerable<BremsstrahlungEnergyYieldVM> ReadEnergyIntensityFile(string filename, char delimeter = '\t')
        {
            try
            {
                ...
                using (StreamReader rd = new StreamReader(filename))
                {
                    ...
                    while ((line = rd.ReadLine()) != null)
                    {
                        ...
                        if (SourceTotalActivity != activity)
                            SourceTotalActivity = activity;

                        var bey = new BremsstrahlungEnergyYieldVM(this) { Energy = energy, EnergyYield = energyYield, EnergyFlux = energyFlux, PhotonsFlux = photonsFlux };
                        items.Add(bey);
                    }
                }
                return items;
            ...
            return Enumerable.Empty<BremsstrahlungEnergyYieldVM>();
        } 
        #endregion
